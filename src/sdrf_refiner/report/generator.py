...
class ReportGenerator:
    ...
    def _format_ptm_hit(
        ...
        validated = hit.get("total_validated")
        validated_str = f" ({validated} validated)" if validated is not None else ""
        ...
        return (
            f"{name}{delta_str} ({unimod}): {evidence} unique pairs{validated_str} "
    ...
    def _add_footer(self) -> None:
        """Add report footer."""
        self._add_line(self.SEPARATOR)
        self._add_line(f"Generated by sdrf-refiner v{__version__}")
        self._add_line(self.SEPARATOR)
